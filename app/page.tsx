...
export default function Home() {
  ...
  const progressStep = !selectedShow ? 1 : !hasDates ? 2 : booking.adults < 1 ? 3 : !selectedRoom ? 4 : 5;
  ...
  function buildRequestMessage() {
    ...
    return [
      "Здравствуйте! Хотим проверить отдых в «Санрайзе».",
      "",
      `Программа: ${selectedProgram}`,
      `Дата программы: ${selectedShow?.date ? formatDate(selectedShow.date) : "уточняется под даты проживания"}`,
      `Заезд: ${booking.checkIn ? formatDate(booking.checkIn) : "не указан"}`,
      `Выезд: ${booking.checkOut ? formatDate(booking.checkOut) : "не указан"}`,
      `Гости: ${guests}`,
      `Номер: ${selectedRoom?.name ?? "категория уточняется"}`,
  ...
  const faq = [
    { question: "Какая программа будет в мои даты?", answer: "Ближайшие текущие даты на официальной странице шоу пока не опубликованы. Укажите период проживания — сотрудник проверит программу под этот заезд.", source: sunriseConfig.officialLinks.shows, status: "Проверено 13.08.2026 · даты ожидают публикации" },
    { question: "Что входит в музыкальную программу?", answer: "В открытых материалах отеля указаны живая музыка, игры, конкурсы, бармен-шоу, файер-шоу и восточные танцы. Дни и состав конкретного вечера нужно подтвердить.", source: sunriseConfig.officialLinks.shows, status: "Подтверждено · детали вечера уточняются" },
    { question: "Какие номера можно проверить?", answer: "Опубликованы пять семейных категорий: 2местный, 3местный и 4местный с балконом, 2-комнатный семейный с балконом и 3местный семейный без балкона.", source: sunriseConfig.officialLinks.allRooms, status: "Подтверждено 13.08.2026" },
    { question: "Что известно о питании?", answer: "Официальная страница питания указывает формат «шведский стол», «всё включено», завтрак, обед и ужин, а также две открытые зоны питания.", source: sunriseConfig.officialLinks.food, status: "Подтверждено 13.08.2026" },
    { question: "Есть ли бассейн и пляж?", answer: "В официальных материалах указаны тёплый бассейн с детским отделением и собственный пляж. Глубину детской зоны не приводим: на страницах отеля встречаются разные значения.", source: "https://sunrise-hotel.ru/pool/", status: "Бассейн и пляж подтверждены · глубина в конфликте" },
    { question: "Показываются ли цены?", answer: "На странице отеля есть таблица стоимости 2026, но доступность и условия нужно проверить под конкретные даты. Поэтому здесь отображается «Стоимость уточняется по выбранным датам», без неподтверждённых чисел.", source: sunriseConfig.officialLinks.prices, status: "Цены требуют проверки под запрос" },
    { question: "Отправляется ли заявка автоматически?", answer: "Нет. После заполнения сайт формирует структурированный текст, который можно скопировать и передать в официальный канал отеля. Подтверждение доступности и стоимости происходит после ответа сотрудника.", source: sunriseConfig.officialLinks.book, status: "Форма отеля и каналы проверены 13.08.2026" },
  ];
  ...
  return (
    <main className="sunrise-page">
      ...
      <section id="top" className="hero-section">
        ...
        <div className="hero-content page-width">
          ...
          <aside className="hero-program-card" aria-label="Расписание шоу">
            ...
            <p className="card-eyebrow">Ближайшие шоу</p>
            <h2>Расписание ближайших шоу уточняется</h2>
            <p>Точные даты текущей программы не опубликованы. Оставьте период проживания — сотрудник проверит вечер под ваш заезд.</p>
            <button type="button" className="card-link" onClick={beginRequest}>Указать даты проживания <span aria-hidden="true">↗</span></button>
            <small className="checked-note">Проверено: 13 августа 2026</small>
          </aside>
        </div>
      ...
      <section className="intro-band"><div className="page-width intro-grid"><p className="eyebrow">Как это работает</p><div><h2>Сначала — настроение.<br /><em>Потом — даты.</em></h2><p className="section-copy">Выберите опубликованную программу или оставьте период проживания. Сохраним контекст и проверим условия именно для вашего заезда.</p></div><div className="intro-note"><span>↘</span><p>Один запрос<br />с понятным контекстом</p></div></div></section>

      <section id="calendar" className="calendar-section section-dark page-width-section"><div className="page-width"><SectionIntro eyebrow="01 / Программа" title="Ближайшие шоу" copy="Календарь использует текущий месяц и будущие месяцы. Если дата ещё не опубликована, оставьте период проживания — сотрудник проверит программу под ваш заезд." light /><div className="calendar-toolbar"><div className="month-switcher" role="tablist" aria-label="Выбор месяца">{calendarMonths.map((month) => <button key={month.key} type="button" role="tab" aria-selected={calendarMonth === month.key} className={calendarMonth === month.key ? "active" : ""} onClick={() => setCalendarMonth(month.key)}>{month.shortLabel}</button>)}</div><p className="calendar-status"><span className="status-ring" /> Проверено 13 августа 2026</p></div>{confirmedShows.length > 0 ? <div className="show-grid">{confirmedShows.map((show) => <article className="show-card" key={show.id}><div className="show-card-image"><LocalImage src={show.image ?? sunriseConfig.gallery[1].src} alt={show.title} /></div><div className="show-card-body"><p className="eyebrow">{show.type}</p><h3>{show.title}</h3><p>{show.description}</p><div className="show-meta">{show.date ? formatDate(show.date) : "Дата уточняется"}{show.time ? ` · ${show.time}` : ""}</div><button type="button" className="text-button" onClick={() => chooseShow(show)}>Выбрать эту программу <span aria-hidden="true">↗</span></button></div></article>)}</div> : <div className="calendar-empty"><div className="calendar-orbit" aria-hidden="true"><span>♪</span><span>✦</span><span>◌</span></div><div className="empty-copy"><p className="eyebrow">{selectedMonth?.label ?? "Ближайший месяц"} · программа</p><h3>Ближайшие даты шоу уточняются</h3><p>На официальной странице нет текущих ISO-дат программы. Укажите даты проживания — сотрудник проверит, какой вечер доступен в ваш период.</p><div className="empty-actions"><button type="button" className="button button-primary" onClick={beginRequest}>Проверить программу на мои даты <span aria-hidden="true">↗</span></button><a className="source-link source-link-light" href={sunriseConfig.officialLinks.shows} target="_blank" rel="noreferrer">Открыть официальный раздел шоу ↗</a></div></div><div className="empty-side-note"><span className="quote-mark">“</span><p>Вы выбираете<br />не просто номер,<br /><em>а повод приехать.</em></p></div></div>}<div className="calendar-facts"><span>Живая музыка</span><span>Игры и конкурсы</span><span>Анимационные шоу</span><span>На территории отеля</span></div></div></section>

      <section id="program-detail" className="detail-section page-width-section"><div className="page-width detail-grid"><div className="detail-copy"><p className="eyebrow">02 / Выбранная программа</p><h2>{selectedShow ? selectedShow.title : "Программа под ваш заезд"}</h2><p className="section-copy">{selectedShow ? selectedShow.description : "Даты конкретного шоу ещё не опубликованы. Оставьте желаемый период — запрос сохранит контекст и передаст его сотруднику отеля."}</p><div className="detail-tags"><span>{selectedType}</span><span>{selectedShow?.date ? formatDate(selectedShow.date) : "Дата уточняется"}</span></div><div className="detail-list"><div><span>Что подтверждено</span><p>{selectedShow?.known.join(" · ") ?? sunriseConfig.showFormat}</p></div><div><span>Что проверим</span><p>{selectedShow?.toClarify.join(" · ") ?? "Дата, время, состав программы и доступность размещения."}</p></div></div><button type="button" className="button button-dark" onClick={beginRequest}>Подобрать проживание под эту программу <span aria-hidden="true">↗</span></button></div><div className="detail-visual"><div className="detail-image"><LocalImage src={selectedShow?.image ?? sunriseConfig.gallery[1].src} alt={selectedShow?.title ?? "Официальная фотография шоу-программы «Санрайза»"} /><div className="detail-image-caption"><span>Санрайз</span><span>Витязево</span></div></div><div className="detail-stamp" aria-hidden="true">stay<br /><i>for</i><br />the<br /><strong>evening</strong></div></div></div></section>

      <section id="request" className="request-section section-dark page-width-section"><div className="page-width"><SectionIntro eyebrow="03 / Даты и гости" title="Подберите заезд под программу" copy="Сначала зафиксируем даты и состав гостей, затем выберем номер и сформируем запрос." light /><div className="request-layout"><div className="request-form-card"><div className="stepper" aria-label={`Шаг ${progressStep} из 5`}><div className={progressStep >= 1 ? "done" : ""}><span>01</span><small>Программа</small></div><i /><div className={progressStep >= 2 ? "done" : ""}><span>02</span><small>Даты проживания</small></div><i /><div className={progressStep >= 3 ? "done" : ""}><span>03</span><small>Гости</small></div><i /><div className={progressStep >= 4 ? "done" : ""}><span>04</span><small>Номер</small></div><i /><div className={progressStep >= 5 ? "done" : ""}><span>05</span><small>Заявка</small></div></div><div className="selected-context"><span className="context-icon">✦</span><div><span>Выбранная программа</span><strong>{selectedProgram}</strong><small>{selectedShow?.date ? formatDate(selectedShow.date) : "Дата и формат уточняются сотрудником"}</small></div><button type="button" onClick={() => scrollToId("calendar")}>Изменить</button></div><div className="fields-grid"><label><span>Дата заезда</span><input type="date" min={today} value={booking.checkIn} onChange={(event) => updateBooking("checkIn", event.target.value)} /></label><label><span>Дата выезда</span><input type="date" min={booking.checkIn || today} value={booking.checkOut} onChange={(event) => updateBooking("checkOut", event.target.value)} />{dateError ? <small className="field-error">{dateError}</small> : null}</label><label><span>Взрослые</span><select value={booking.adults} onChange={(event) => updateBooking("adults", Number(event.target.value))}>{[1, 2, 3, 4, 5, 6].map((value) => <option key={value} value={value}>{value} {guestLabel(value, "взрослый", "взрослых", "взрослых")}</option>)}</select></label><label><span>Дети</span><select value={booking.children} onChange={(event) => updateBooking("children", Number(event.target.value))}>{[0, 1, 2, 3, 4].map((value) => <option key={value} value={value}>{value} {guestLabel(value, "ребёнок", "ребёнка", "детей")}</option>)}</select></label></div><label className="full-field"><span>Пожелания к заезду</span><textarea rows={3} placeholder="Например, хочется попасть на вечер с живой музыкой" value={booking.comment} onChange={(event) => updateBooking("comment", event.target.value)} /></label><button type="button" className="button button-primary button-wide" onClick={() => { track("dates_selected", { check_in: booking.checkIn, check_out: booking.checkOut }); continueRequest(); }}>{selectedShow ? hasDates ? selectedRoom ? "Перейти к заявке" : "Выбрать номер" : "Сохранить даты" : "Выбрать программу"} <span aria-hidden="true">↓</span></button></div><aside className="request-summary" aria-live="polite"><p className="eyebrow">Ваш запрос</p><h3>Проверим<br /><em>вечер под заезд</em></h3><div className="summary-line"><span>Программа</span><strong>{selectedProgram}</strong></div><div className="summary-line"><span>Период проживания</span><strong>{formatDateRange(booking.checkIn, booking.checkOut)}</strong></div><div className="summary-line"><span>Гости</span><strong>{booking.adults} {guestLabel(booking.adults, "взрослый", "взрослых", "взрослых")} · {booking.children} {guestLabel(booking.children, "ребёнок", "ребёнка", "детей")}</strong></div><div className="summary-line"><span>Номер</span><strong>{selectedRoom?.name ?? "Категория ещё не выбрана"}</strong></div><div className="summary-note"><span>i</span><p>Это запрос на проверку доступности, а не подтверждённое бронирование.</p></div></aside></div></div></section>

      <section id="stay" className="stay-section page-width-section"><div className="page-width"><SectionIntro eyebrow="04 / Номер" title="Выберите вариант проживания" copy="Карточки используют названия и параметры официального номерного фонда. Стоимость и наличие проверяются по выбранным датам." /><div className="rooms-grid">{sunriseConfig.rooms.filter((room) => room.confirmed).map((room) => <article className={`room-card${selectedRoomId === room.id ? " room-card-selected" : ""}`} key={room.id}><div className="room-image-wrap"><LocalImage src={room.photo} alt={room.photoAlt} className="room-image" /><span className="room-availability">{selectedRoomId === room.id ? "Выбрано" : "Под даты"}</span></div><div className="room-card-body"><div className="room-card-top"><span className="room-icon">{room.name.charAt(0)}</span><span className="room-area">{room.area}</span></div><h3>{room.name}</h3><p>{room.details}</p><div className="room-specs"><span>{room.capacity}</span><span>{room.balcony} · {room.floors}</span><span>{room.food}</span></div><div className="room-divider" /><span className="room-price">{room.priceLabel}</span><small>{room.conditions} · проверено {room.priceCheckedAt}</small><a className="source-link" href={room.officialUrl} target="_blank" rel="noreferrer">Описание на сайте отеля ↗</a><button type="button" className={`text-button${selectedRoomId === room.id ? " text-button-selected" : ""}`} onClick={() => chooseRoom(room.id)}>{selectedRoomId === room.id ? "Выбрано · изменить" : "Выбрать номер"} <span aria-hidden="true">↗</span></button></div></article>)}</div></div></section>

      <section className="included-section section-soft page-width-section"><div className="page-width"><SectionIntro eyebrow="05 / Условия" title="Что подтверждено программой и проживанием" copy="Практические условия, которые можно учесть при выборе дат. Детали конкретного предложения зависят от периода и подтверждаются сотрудником." /><div className="included-grid">{sunriseConfig.includedItems.filter((item) => item.confirmed).map((item) => <article className="included-card" key={item.title}><span className="included-number">{item.icon}</span><p className="eyebrow">{item.eyebrow}</p><h3>{item.title}</h3><p>{item.description}</p><a className="source-link" href={item.sourceUrl} target="_blank" rel="noreferrer">Источник ↗</a></article>)}</div></div></section>
